...
handle_image() {
    ...
    case "${mimetype}" in
        ...
        # DjVu
        image/vnd.djvu)
            timeout 3s ddjvu -format=tiff -quality=90 -page=1 -size="${DEFAULT_SIZE}" \
        ...
        image/*)
        #   local orientation                                                                         #
        #   orientation="$( timeout 3s identify -format '%[EXIF:Orientation]\n' -- "${FILE_PATH}" )"  #    NOTE: temporarily disabled:
        #   ## If orientation data is present and the image actually                                  #    is this rotation
        #   ## needs rotating ("1" means no rotation)...                                              #    stuff really needed?
