...
-------------------------------------------------------------------
# Install tree utility. It's really useful
...
# My main email account is the login.
...
-------------------------------------------------------------------
# Download and install gxneur package.
...
-------------------------------------------------------------------
# Ubuntu can have wireless disabled after booting. To overcome this:
...
-------------------------------------------------------------------
# If using notebook Ubuntu always sets max brightnes on startup. Set the startup level of
# brightness:
sudoedit /etc/rc.local
...
-------------------------------------------------------------------
# If using notebook Ubuntu changes brightness too dramatically with each iteration. To make
# the changes smoother:
...
# Go to Keyboard Layout, select Options...
# Caps Lock behaviour -> disabled
...
-------------------------------------------------------------------
# Russian layout may not work by default. To enable it go to Keyboard Layout
# Press on plus sign and select Russian in the list
...
-------------------------------------------------------------------
# If using notebook and Ubuntu to make it do nothing on lid closing in /etc/UPower/UPower.conf
# set option Ignored=true
...
-------------------------------------------------------------------
# If using a notebook, configure the power options to prevent hibernating when closing the laptop lid


-------------------------------------------------------------------
# Synchronize time with one of the servers. Default preset likely to be wrong due to stupid russian government that always changes time.
...
-------------------------------------------------------------------
# There may be an issue with brightness while installing Ubuntu - the screen may be
# completely dark but the installation process will not be interrupted. In such cases just
# volume brightness up.


-------------------------------------------------------------------
# Change keybindings for moving between workspaces. There will be a problem with 4-buttons
# shortcuts, so install dconf-editor package.
sudo apt-get install dconf-editor
Run it and go through the path:
...
-------------------------------------------------------------------
# If not using Ubuntu you may need to install and setup icedove. Create all the existing email
# accounts. Maybe there is some way to export them from already existing icedove configuration
sudo apt-get install icedove


-------------------------------------------------------------------
# Set up auto save when taking screenshots. To do this, install scrot:
...
-------------------------------------------------------------------
# If using a notebook and having default behaviour of Function keys such that you have to
# press Fn to access F1-F12 keys, do the following:
...
-------------------------------------------------------------------
# On Ubuntu remove shitty packages
remove totem
remove totem-common
remove rhythmbox
...
-------------------------------------------------------------------
# If Ubuntu notifies about critical battery level, but in reality the battery is fully charged, change the following in dconf-editor:

# Go to:
# org > gnome > settings-daemon > plugins > power

# And change the value of critical-battery-action to nothing

# And run this command to turn this annoying notification off
gsettings set org.gnome.settings-daemon.plugins.power 'use-time-for-policy' 'false'


-------------------------------------------------------------------
# To make Ubuntu dash search only for applications, run following command:
...
# Just download the .deb file and run it: http://blogs.unity3d.com/2015/08/26/unity-comes-to-linux-experimental-build-now-available/


# It's better to install it through PlayOnLinux
# There's a good working guide here:
# http://wiki.unity3d.com/index.php/Running_Unity_on_Linux_through_Wine#Installation_with_PlayOnLinux

# Installation without PlayOnLinux (obsolete)
# wine and winetricks has to be installed
# NOT VERIFIED: Before doing anything, wine should be configured to work in 32 bit mode. Steps to make this:
# export WINEARCH=win32
# Move current ~/.wine directory somewhere
# The do all the following steps. And only after all steps will be implemented, run unity
# VERIFIED:
winetricks tahoma
winetricks d3dx9
winetricks dotnet20
...
mkdir -p ~/.wine/drive_c/users/mikhail/AppData/LocalLow
...
-------------------------------------------------------------------
# If you're having problems with sound in wine. In Ubuntu 12.04 install package:
sudo apt-get install ia32-libs-multiarch
# And restart pc


-------------------------------------------------------------------
# Disable Alt behavior. Go to Keyboard. And disable "Key To show the HUD"
...
# In order to make the key to work permanently you may also need to manually specify each key in your ~/.ssh/config like this:
